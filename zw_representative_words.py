...
def main(data_file, seed):
    ...
    probabilistic_score_dict = probabilistic_score(pos_rev5=pos_rev5, pos_rev4=pos_rev4,
                                                    neg_rev2=neg_rev2, neg_rev1=neg_rev1)
    ...
    pd.DataFrame.from_dict(orderd_word_score_dict, orient="index").to_csv("./data/rep_words.csv")

    # writeout = []

    # for key, value in orderd_word_score_dict.items():
    #     writeout.append((key, value))

    # with open("./data/rep_words.csv", "w") as outfile:
...
def probabilistic_score(pos_rev5, pos_rev4, neg_rev2, neg_rev1):
    ...
    n_pos = sum(pos_count5.values()) + sum(pos_count4.values())  # npos
    n_neg = sum(neg_count2.values()) + sum(neg_count1.values())  # nneg
    n = n_pos + n_neg
    ...
    # for word in words:
    #     try:
    #         p_word_pos_dict[word] = (((4 * pos_count5.get(word, 0) + pos_count4.get(word, 0)) / n_pos) + 1) / (kpos + 1)
    #     except ZeroDivisionError:
    #         p_word_pos_dict[word] = 0

    #     try:
    #         p_word_neg_dict[word] = (((4 * neg_count1.get(word, 0) + neg_count2.get(word, 0)) / n_neg) + 1) / (kneg + 1)
    #     except ZeroDivisionError:
    #         p_word_neg_dict[word] = 0
    ...
    # for word in words:
    #     try:
    #         p_pos_word_dict[word] = (p_pos * p_word_pos_dict[word]) / ((pos_count5.get(word, 0) + pos_count4.get(word, 0) + neg_count2.get(word, 0) + neg_count1.get(word, 0)) / n)
    #     except ZeroDivisionError:
    #         p_pos_word_dict[word] = 0

    #     try:
    #         p_neg_word_dict[word] = (p_neg * p_word_neg_dict[word]) / ((neg_count2.get(word, 0) + neg_count1.get(word, 0) + pos_count5.get(word, 0) + pos_count4.get(word, 0)) / n)
    #     except ZeroDivisionError:
    #         p_neg_word_dict[word] = 0

    score_dict = dict()
    for word in words:
        # score_dict[word] = round(1/(1 + math.exp(p_pos_word_dict[word] - p_neg_word_dict[word])), 3)
